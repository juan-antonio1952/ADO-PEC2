...
El flujo de trabajo (*workflow*) ha sido organizado de la siguiente forma:
...
NOTA: En la información disponible en el archivo ``targets`` aparecen muestras obtenidas mediante ultrasecuenciación y otras técnicas. Se considera que ambas son válidas y se utilizan todas las seleccionadas.
...
Se observa que el archivo final tiene datos de 30 muestras correspondientes a 56202 tránscritos.
...
Se muestra una selección del contenido de los tres.

En el objeto ``samples`` se renombran la variable grupo según el grupo de muestras (NIT, SIF, ELI) y se presenta una selección que cubre los tres grupos.
...
El número de tránscritos se había reducido de 56202 a 46387 al eliminar las filas con todos sus valores a cero; con el filtrado el tamaño (número de filas) se ha reducido a 22185.
...
Un *volcano plot* es una manera efectiva de presentar un test estadístico. Se trata de un gráfico de dispersión con el $\textrm{log2}$ de la diferencia de medias (el numerador del test $t$ o *fold change*) en el eje de las x y los valores $p$ en el eje de las y. El gráfico termina adoptando la forma de volcán si se transforman los valores $p$ aplicando la función $\textrm{−log10}(p)$. Para destacar genes, se añaden dos rectas verticales y una recta horizontal. Teniendo en cuenta lo que se presenta en cada eje, si se quieren destacar los genes con un valor $p$ por debajo de 0.01 se coloca la recta horizontal en 2: los genes destacados son lo que quedan por encima de la recta. El $\textrm{log2}$ del FC (*fold change*) se usa para mostrar los genes equidistantes del centro que quedan arriba (*up*) y abajo (*down*).
...
```{r fig.height=6, fig.width=6, fig.cap='Figura 20. Diagrama de Venn con las tres comparaciones - Segundo método.', echo=FALSE}
library(VennDiagram)
#par(mar=c(4,30,4,30))
...
```{r fig.height=6, fig.width=6, fig.cap='Figura 21. Diagrama de Venn con las tres comparaciones - Tercer método.', echo=FALSE}
...
Para visualizar los resultados de cada conjunto de genes se puede utilizar la opción ``barcodeplot`` que presenta los genes correspondientes al término GO seleccionado ordenados de izquierda a derecha con un valor creciente de logFC.
